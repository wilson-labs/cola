...
colors = {"cola": "#2b8cbe", "GPyTorch": "#e34a33", "sklearn": "#8856a7", "SciPy": "#7fc97f"}

gps_gpytorch = np.array([0.1, 0.1])
gps_cola = np.array([0.11, 0.12])

minimal_scipy = np.array([0.34, 0.5])
minimal_cola = np.array([0.15, 0.2])

spectral_sklearn = np.array([0.6, 0.7])
spectral_cola = np.array([0.15, 0.18])
...
label = "cola"
plt.bar(br2, np.mean(gps_cola[skip:]), color=colors[label],
...
plt.bar(br1, np.mean(spectral_sklearn[skip:]), color=colors[label],
        width=barWidth, edgecolor='black', label=label, lw=4)
label = "cola"
plt.bar(br2, np.mean(spectral_cola[skip:]), color=colors[label],
...
plt.bar(br1, np.mean(minimal_scipy[skip:]), color=colors[label],
        width=barWidth, edgecolor='black', label=label, lw=4)
label = "cola"
plt.bar(br2, np.mean(minimal_cola[skip:]), color=colors[label],
        width=barWidth, edgecolor='black', lw=4)
