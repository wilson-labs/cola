...
mask = (df["case"] == "cola") & (df["ds_size"] == 10)
cola1 = np.array(df[mask]["times"])

mask = (df["case"] == "sklearn") & (df["ds_size"] == 25)
sk2 = np.array(df[mask]["times"])
mask = (df["case"] == "cola") & (df["ds_size"] == 25)
cola2 = np.array(df[mask]["times"])

mask = (df["case"] == "sklearn") & (df["ds_size"] == 50)
sk3 = np.array(df[mask]["times"])
mask = (df["case"] == "cola") & (df["ds_size"] == 50)
cola3 = np.array(df[mask]["times"])
...
plt.bar(br2, np.mean(cola1[skip:]), color=colors[0], width=barWidth, edgecolor='black',
        label="CoLA", lw=4)
...
plt.bar(br2, np.mean(cola2[skip:]), color=colors[0], width=barWidth, edgecolor='black', lw=4)

start += 2. * barWidth + barWidth / 2.
br1 = [start]
br2 = [x + barWidth for x in br1]
plt.bar(br1, np.mean(sk3[skip:]), color=colors[-1], width=barWidth, edgecolor='black', lw=4)
plt.bar(br2, np.mean(cola3[skip:]), color=colors[0], width=barWidth, edgecolor='black', lw=4)
